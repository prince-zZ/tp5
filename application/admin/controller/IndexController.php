...
class IndexController extends AdminAuth
{
    ...
    public function login_action(){
        ...
        if ($user = $user->where($where_query)->find()) {
            //注册session
            $list = Db::table('think_rbac')->field("name,title")->where("id = '$user->rbac'")->find();
            ...
            if($list){
                ...
                session('rbac',encrypt(encrypt($list['title'],"ENCODE"),"ENCODE"),'think');

                //更新最后请求IP及时间
                $time = time();
                $expire_time = time()+config('auth_expired_time');
                $user->where($where_query)->update(['last_login_ip' => get_ip(), 'last_login_time' => $time,'expire_time'=>$expire_time]);
                $datas['name'] = $user->username.'|登录后台';
                $datas['ip'] = get_ip();
                $datas['add_time'] = time();
                Db::table("think_journal")->insert($datas);
     ...
     */
    public function lost_password(){die;
        $this->view->engine->layout(false);
        return view();
    }
    /**
    ...
    public function logout(){
        $datas['name'] = session('admin_username').'|退出后台';
