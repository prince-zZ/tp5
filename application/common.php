...
/**
 * encrypt function.
 * This feature encryption and decryption
 *
 * @author xiaozhou
 * @access public
...
function encrypt($string, $operation = 'ENCODE', $key = '', $expiry = 0){
	...
	if(defined("G_BANBEN_NUMBER")){
		$key = md5($key != '' ? $key : $_SERVER["REQUEST_SCHEME"].'://'.$_SERVER["SERVER_NAME"]);
	}
	$fixedkey = md5($key);
...
/**
 * delete function.
 * Recursive deletion of this feature
 *
 * @author xiaozhou
 * @access public
 * @param string   
...
function rmdirr($dirname) {
    ...
    $dir->close();
    return @rmdir($dirname,0777,true);
}
